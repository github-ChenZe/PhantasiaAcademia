...
\end{corollary}
\begin{remark}
	注意与\eref{无限大平面上均匀分布的电荷产生的电场}中的结论区分, 这其实显示了外界电场对电荷分布的影响.
\end{remark}
参考\eref{无限大平面上均匀分布的电荷产生的电场}可以发现外界电场(除了$\rd{a}$上的电荷之外的源产生的电场)为$\vE' = \sigma\hat{\vn}/\pare{2\epsilon_0}$.
\begin{finale}
...
\begin{proof}
	...
	\begin{cenum}
		...
		\item 只能源于$+q$.
	\end{cenum}
	则存在$+q$附近的非封闭曲面满足$\iint_{q_{-}} \vE\cdot\rd{a} = \iint_{S} \vE\cdot\rd{a} < \oiint_{+q} \vE\cdot\rd{a}$. 最后一步的积分也可以用\Cref{电场线的性质}替代.
\end{proof}
\begin{ex}
...
\begin{ex}
	...
	将\eref{点电荷在导体上感应}中的导体接地, 设地面电势$\varphi_G = \varphi_\infty = 0$, 则导体上没有点的$\sigma > 0$, 否则导体与地面不等势. 此外, 如果接地导体外是带电导体, 则带电导体的电荷处处为正, 否则外导体不等势.
\end{ex}
定量分析在足够对称的条件下也是可行的.
\begin{ex}
...
\end{ex}
\begin{ex}
	参考\eref{球外有点电荷的导体球的电势}, 接地导体球上的感应电荷为
	\[ q' = -\frac{q}{R}{l}. \]
\end{ex}
\begin{ex}
	无限大接地金属版外$l$处有点电荷$q$, 则金属板处的电势和不接地而在$-l$处设定电荷$-q$的结果相当. 参考\eref{平行无限大平面上均匀分布的异号电荷产生的电场}, 金属版上
	\[ E\pare{r} = \rec{2\pi\epsilon_0}\frac{q}{r^2}\cos^3\theta,\quad \sigma = \frac{q}{2\pi l^2}\cos^3\theta. \]
	注意到接地导致右壁电荷密度为零(\eref{导体外有点电荷的接地导体的表面电荷密度}). 通过导体内$E=0$仔细计算$\sigma$也会得到同样的结果.
\end{ex}
\begin{ex}
...
\end{finale}
\begin{ex}
	如果金属壳不接地, 则由\eref{封闭金属壳外表面总电荷}, 金属壳外的电场可能于壳内电荷相关.
\end{ex}
...
\end{ex}
\begin{ex}
	如果平方反比定律(\axref{Coulomb定律})不成立, 则\cref{Gauss定律}不能成立, 因此均匀带电球壳内部电场非零. 如果用导线将球壳与球壳内部的导体连接, 内部导体会带电. 事实证明没有这种带电, 从而(精确地)验证了平方反比定律.
\end{ex}
...
对于孤立导体,参考\eref{均匀带电球面的电势}可以发现$q=CV$, 其中$q$为常数. 对于两个带相反电荷的导体, 之间的电压为
\begin{finale}
...
\begin{ex}
	用带电导体A感应导体B, 将B近端分开知B将带电. 或将B接地随后移除A, 由\eref{导体外有点电荷的接地导体的表面电荷密度}知B将带电.
\end{ex}
\begin{ex}
...
\begin{ex}
	...
	求和对每个导体分别进行, 因为导体是等势的. 由\eref{导体外有点电荷的接地导体的表面电荷密度}后段, 如果一导体接地而另一导体带电, 则两导体之间纯吸引, 因此该情形下两导体之间的电势与距离正相关.
\end{ex}
可以认为公式对电容器也适用, 从而
